...
class ClickButtonByPoetryFillAction(SignAction):
    action: Literal[SupportAction.CLICK_BUTTON_BY_POETRY_FILL] = (
        SupportAction.CLICK_BUTTON_BY_POETRY_FILL
    )


class AssertSuccessByTextAction(SignAction):
    action: Literal[SupportAction.ASSERT_SUCCESS_BY_TEXT] = (
    ...
    def __str__(self) -> str:
        ...
        for i, action in enumerate(self.actions, 1):
            ...
            if isinstance(action, SendTextAction):
                ...
                keyword_preview = ", ".join(action.keywords[:2])
                if len(action.keywords) > 2:
                    keyword_preview += ", ..."
                details = f"Assert: {keyword_preview}"
