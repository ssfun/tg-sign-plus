...
等待不依赖机器人回复，可放在开头、结尾或动作之间；期间收到的普通交互消息按执行锁排队处理，命中成功判定的消息会立即结束等待并跳过后续动作。取消任务也会中断等待。内部重试会重新执行入口动作及其中的等待；Worker 与事件引擎统一额外预留全部等待秒数 ×（内部重试次数 + 1）；入口阶段已完成的等待不会再次计入事件阶段预算。RPC 和单个响应动作的超时不变。`sign_interval` 仍只控制不同会话之间的间隔。
